...
from model_utils import run_single_experiment_with_extra_data
...
def get_experiment_config(mode, current_dir):
    ...
    """
    if mode == 'own_pv_bv5_gtv2_current':
        logging.info("Selected Mode: 1. Evaluation own methods V2 with extra feature (Current) on PV Datasets (BV5 GTV2).")
        return {
            "param_grid": DEFAULT_GRID_PV_DS_WITH_EF_CURRENT_FOR_OWN_EVAL,
            "output_dir": current_dir / "datasets" / "processed_own_method_V2_with_ef_current_eval_pv_bv5_gtv2_ds_default",
            "extra_feature":"Current",
            "cols_slice": slice(1, 2049),
            "ds_name": "PV",
            "use_tuning_datasets": False,
            "ds_paths": [current_dir / 'datasets' / 'raw' / 'ScenariosV5_GTV2'],
            "tuning_ds": {"DA3_20250610_095701_ALPS"}
    }

    elif mode == 'sota_pv_bv5_gtv2_current':
        logging.info("Selected Mode: 2. Evaluation SOTA methods with extra feature (Current) on PV Datasets (BV5 GTV2).")
        return {
            "param_grid": DEFAULT_GRID_PV_DS_WITH_EF_CURRENT_FOR_SOTA_EVAL,
            "output_dir": current_dir / "datasets" / "processed_sota_method_with_ef_current_eval_pv_bv5_gtv2_ds",
            "extra_feature":"Current",
            "cols_slice": slice(1, 2049),
            "ds_name": "PV",
            "use_tuning_datasets": False,
        ...
        logging.info("Selected Mode: 3. Running default test configuration with extra feature (Current).")
        return {
            "param_grid": OPTIMAL_GRID_PV_DS_WITH_EF_CURRENT_FOR_OWN_EVAL,
            "output_dir": current_dir / "datasets" / "processed_eval_test_with_ef",
            "extra_feature":"Current",
            "cols_slice": slice(1, 2049),
            "ds_name": "PV",
            "use_tuning_datasets": True,
            "ds_paths": [current_dir / 'datasets' / 'raw' / 'ScenariosV5_GTV2'],
            "tuning_ds": { "RMX_20251116_161440_ALPS"}
        }
    else:
...
def main():
    ...
    modes = [
        'own_pv_bv5_gtv2_current',  # 1. Own method evaluation on PV Datasets (BV5 GTV2)
        'sota_pv_bv5_gtv2_current', # 2. SOTA method evaluation on PV Datasets (BV5 GTV2)
        'test_current'              # 3. Default test mode
    ]
    ...
    parser.add_argument(
        ...
        type=str,
        choices=modes,
        default='test_current', # Set 'test' as the default mode
        help=(
    ...
    extra_feature = config["extra_feature"]
        ...
        for j, params in enumerate(grid):
            ...
            for f, i in product(files_to_process, range(N_RUNS)):
                ...
                if model_name in ["SWKNN", "SWKNN_own", "SWLOF", "KitNet", "ExactStorm", "mDragStream"] and i > 0:
                    continue
                """
                task_config = (f, p_window_size, model_name, params, i, j, output_dir, cols_slice, extra_feature, ds_name)
    ...
    if tasks:
        for i, task in enumerate(tasks):
            ...
            try:
               result_message = run_single_experiment_with_extra_data(task)
